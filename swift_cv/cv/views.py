...
def create_cv(request):
    if request.method == "POST":
        ...
        for i in inst_name:
               ...
               inst  = Institution.objects.create(cv_id=u, inst_name=inst_name,result=result)
               inst.save() 

        for j in skill:
        
               skill  = Skill.objects.create(cv_id=u, skill_name=skill)
               skill.save() 
        for k in ach:
        
               ach  = Achievement.objects.create(cv_id=u, ach_name=ach)
               ach.save() 
